...
from core.sfm_models import (
    SFMGraph, Actor, Institution, Resource, Policy, Flow, Relationship, 
    Node, RelationshipKind
)
from core.enums import ResourceType, FlowNature
from core.sfm_query import (
    SFMQueryEngine, NetworkXSFMQueryEngine, SFMQueryFactory,
    AnalysisType, QueryResult, NodeMetrics, FlowAnalysis
)
...
class TestNetworkXSFMQueryEngineUnit(unittest.TestCase):
    ...
    def test_get_node_centrality_betweenness(self, mock_centrality):
        ...
        centrality = self.query_engine.get_node_centrality(self.actor1.id, "betweenness")
    ...
    def test_get_relationship_strength(self):
        ...
        strength = self.query_engine.get_relationship_strength(self.actor1.id, self.actor2.id)
        self.assertEqual(strength, 0.8)  # Weight of rel1
        
        # Test non-existing relationship
        no_strength = self.query_engine.get_relationship_strength(self.actor1.id, uuid.uuid4())
    ...
    def test_calculate_flow_efficiency(self):
        ...
        with patch.object(self.query_engine, 'find_shortest_path') as mock_path:
            ...
            efficiency = self.query_engine.calculate_flow_efficiency(self.actor1.id, self.actor2.id)
    ...
    def test_analyze_policy_impact(self, mock_centrality, mock_density, mock_ego_graph):
        ...
        self.assertEqual(impact['total_affected_nodes'], 2)  # Excluding policy node itself
    ...
    def test_comprehensive_node_analysis(self):
        ...
        with patch.object(self.query_engine, 'get_node_centrality') as mock_centrality:
            mock_centrality.side_effect = [0.8, 0.6, 0.7]  # betweenness, closeness, degree
    ...
    def test_system_vulnerability_analysis(self, mock_components, mock_clustering):
        ...
        with patch.object(self.query_engine, 'get_network_density') as mock_density:
            ...
            with patch.object(self.query_engine, 'get_structural_holes') as mock_holes:
                ...
                with patch.object(self.query_engine, 'identify_bottlenecks') as mock_bottlenecks:
...
class TestNetworkXSFMQueryEngineIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.actors = [
            Actor(label=f"Actor {i}", sector="Test") for i in range(5)
        ]
        self.institutions = [
            Institution(label=f"Institution {i}") for i in range(3)
        ]
        self.resources = [
            Resource(label=f"Resource {i}", rtype=ResourceType.NATURAL) for i in range(2)
        ]
        self.policies = [
        ...
        # Add all nodes
        for node_list in [self.actors, self.institutions, self.resources, self.policies]:
        ...
        relationships = [
            ...
            Relationship(self.actors[0].id, self.institutions[0].id, RelationshipKind.GOVERNS, 0.8),
            Relationship(self.actors[1].id, self.institutions[1].id, RelationshipKind.SERVES, 0.7),
            
            # Actors to resources
            Relationship(self.actors[2].id, self.resources[0].id, RelationshipKind.USES, 0.6),
            Relationship(self.actors[3].id, self.resources[1].id, RelationshipKind.PRODUCES, 0.9),            
            # Policies to actors
            Relationship(self.policies[0].id, self.actors[0].id, RelationshipKind.AFFECTS, 0.5),
            Relationship(self.policies[1].id, self.actors[4].id, RelationshipKind.FUNDS, 0.8),
            
            # Cross connections for more interesting topology
            Relationship(self.actors[0].id, self.actors[1].id, RelationshipKind.COLLABORATES_WITH, 0.4),
            Relationship(self.institutions[0].id, self.resources[0].id, RelationshipKind.GOVERNS, 0.7),
        ]
    ...
    def test_real_shortest_path_finding(self):
        ...
        path = self.query_engine.find_shortest_path(self.actors[0].id, self.institutions[0].id)
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_nonexistent_node_queries(self):
        ...
        with self.assertRaises(NetworkXError):
            engine.get_node_neighbors(fake_id) # Should raise Error for non-existent node
