...
from core.enums import (ValueCategory,InstitutionLayer,ResourceType,FlowNature,RelationshipKind)
...
class TestFeedbackLoop(unittest.TestCase):
    ...
    def test_feedback_loop_init(self):
        ...
        self.assertEqual(
            loop.description, "Reinforcing feedback between investment and growth"
        )
...
class TestMetadataClasses(unittest.TestCase):
    ...
    def test_analytical_context_init(self):
        ...
        self.assertEqual(
            context.description, "Analysis of climate policy impacts on economy"
        )
...
class TestRelationship(unittest.TestCase):
    ...
    def test_relationship_init(self):
        ...
        rel = Relationship(
            source_id=source_id, target_id=target_id, kind=RelationshipKind.GOVERNS
        )
...
class TestSFMGraph(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.institution = Institution(
            label="Carbon Tax", layer=InstitutionLayer.FORMAL_RULE
        )
    ...
    def test_add_node(self):
        ...
        self.assertEqual(
            self.graph.institutions.get(self.institution.id), self.institution
        )
    ...
    def test_add_relationship(self):
        ...
        self.assertEqual(storedRel.id, self.relationship.id)  # type: ignore None is default for get method of dict
    ...
    def test_add_feedback_loop(self):
        ...
        rel_ids = [uuid.uuid4() for _ in range(3)]
        loop = FeedbackLoop(
            label="Economic Growth Loop", type="reinforcing", relationships=rel_ids
        )
    ...
    def test_add_analytical_context(self):
        ...
        context = AnalyticalContext(
            label="Climate Policy Analysis", methods_used=["network_analysis"]
        )
    ...
    def test_sfm_graph_iterator(self):
        """Test SFMGraph.__iter__ method thoroughly."""
        graph = SFMGraph()
        ...
        # Test that all node types are included
        node_types = [type(node).__name__ for node in nodes]
        self.assertIn("Actor", node_types)
        self.assertIn("Institution", node_types)
        self.assertIn("Resource", node_types)

    def test_node_iterator_completeness(self):
        """Test that Node.__iter__ includes all attributes."""
        node = Node(
            label="Complete Node",
        ...
        # Should include all Node attributes
        expected_attrs = ["label", "description", "id", "meta"]
        for attr in expected_attrs:
            self.assertIn(attr, attributes)

        # Verify values
        self.assertEqual(attributes["label"], "Complete Node")
        self.assertEqual(attributes["description"], "Full test")
        self.assertEqual(attributes["meta"], {"key1": "value1", "key2": "value2"})
        self.assertIsInstance(attributes["id"], uuid.UUID)

    def test_complete_sfm_graph_operations(self):
        """Test complete SFM graph save/load operations."""
        sfm_graph = SFMGraph(name="Test SFM Graph")

        # Create entities
        actor = Actor(label="Test Actor")
        institution = Institution(label="Test Institution")

        # Add to SFMGraph properly
...
class TestInheritance(unittest.TestCase):
    ...
    def test_policy_institution_inheritance(self):
        """Test that Policy properly inherits from Institution."""
        policy = Policy(label="Test Policy")

        # Test inheritance chain
        self.assertIsInstance(policy, Policy)
        self.assertIsInstance(policy, Institution)
        self.assertIsInstance(policy, Node)

        # Test that Institution methods work on Policy
        self.assertTrue(hasattr(policy, "layer"))

        # Test Policy-specific attributes
        self.assertTrue(hasattr(policy, "authority"))
        self.assertTrue(hasattr(policy, "enforcement"))
        self.assertTrue(hasattr(policy, "target_sectors"))

    def test_all_nodes_inherit_from_node(self):
        """Test that all node types inherit from Node."""
        node_classes = [
            Actor,
            Institution,
            Resource,
        ...
        for node_class in node_classes:
            instance = node_class(label=f"Test {node_class.__name__}")
            self.assertIsInstance(
                instance, Node, f"{node_class.__name__} should inherit from Node"
            )

            # Test that Node attributes are available
            self.assertTrue(hasattr(instance, "id"))
            self.assertTrue(hasattr(instance, "label"))
            self.assertTrue(hasattr(instance, "description"))
            self.assertTrue(hasattr(instance, "meta"))

    def test_relationship_traversal(self):
        """Test traversing relationships in an SFM graph."""
        # Set up a graph with multiple connected nodes
        # Test finding paths, neighbors, etc.

    def test_invalid_relationship_handling(self):
        """Test adding relationships with non-existent nodes."""
        graph = SFMGraph()
        rel = Relationship(
            source_id=uuid.uuid4(),  # Non-existent node
            target_id=uuid.uuid4(),  # Non-existent node
            kind=RelationshipKind.USES,
        )
        # Test how the graph handles this situation

    def test_policy_inheritance_in_graph(self):
        """Test Policy objects are handled correctly as Institution subclass."""
        graph = SFMGraph()
        policy = Policy(label="Climate Policy")
        regular_institution = Institution(label="Regular Institution")

        graph.add_node(policy)
        graph.add_node(regular_institution)

        # Policy should go to policies collection
        self.assertEqual(len(graph.policies), 1)
        self.assertEqual(len(graph.institutions), 1)
        self.assertIn(policy.id, graph.policies)
        self.assertIn(regular_institution.id, graph.institutions)

    def test_dimensional_entity_usage(self):
        """Test TimeSlice, SpatialUnit, and Scenario in relationships."""
        time_slice = TimeSlice(label="2025-Q1")
        spatial_unit = SpatialUnit(code="US-CA", name="California")
        scenario = Scenario(label="High Growth")
        ...
        self.assertEqual(relationship.time.label, "2025-Q1")  # type: ignore  pylance is being an ahole
        self.assertEqual(relationship.space.code, "US-CA")  # type: ignore
        self.assertEqual(relationship.scenario.label, "High Growth")  # type: ignore
