...
from core.sfm_models import (
    Node, Actor, Institution, Resource, Policy, Flow, Relationship,
...
class SFMQueryEngine(ABC):
    ...
    @abstractmethod
    def get_node_centrality(self, node_id: uuid.UUID, 
                           centrality_type: str = "betweenness") -> float:
    ...
    @abstractmethod
    def get_node_neighbors(self, node_id: uuid.UUID, 
                          relationship_kinds: Optional[List[RelationshipKind]] = None,
                          distance: int = 1) -> List[uuid.UUID]:
    ...
    @abstractmethod
    def find_shortest_path(self, source_id: uuid.UUID, target_id: uuid.UUID,
                          relationship_kinds: Optional[List[RelationshipKind]] = None) -> Optional[List[uuid.UUID]]:
        """Find shortest path between two nodes."""
        pass
    
    @abstractmethod
    def get_relationship_strength(self, source_id: uuid.UUID, target_id: uuid.UUID) -> float:
    ...
    @abstractmethod
    def trace_resource_flows(self, resource_type: ResourceType,
                           source_actors: Optional[List[uuid.UUID]] = None) -> FlowAnalysis:
    ...
    @abstractmethod
    def calculate_flow_efficiency(self, source_id: uuid.UUID, target_id: uuid.UUID) -> float:
    ...
    @abstractmethod
    def analyze_policy_impact(self, policy_id: uuid.UUID, 
                            impact_radius: int = 3) -> Dict[str, Any]:
    ...
    @abstractmethod
    def compare_policy_scenarios(self, scenario_graphs: List[SFMGraph]) -> Dict[str, Any]:
    ...
    @abstractmethod
    def identify_communities(self, algorithm: str = "louvain") -> Dict[int, List[uuid.UUID]]:
...
class NetworkXSFMQueryEngine(SFMQueryEngine):
    ...
    def get_most_central_nodes(self, node_type: Optional[type] = None, 
        ...
        sorted_nodes = sorted(filtered_centrality.items(), 
    ...
    def trace_resource_flows(self, resource_type: ResourceType,
        ...
        resource_nodes = [
            ...
            if (isinstance(node_data['data'], Resource) and 
                node_data['data'].rtype == resource_type)
        ]
            ...
            for neighbor in self.nx_graph.neighbors(resource_id):
                for edge_data in self.nx_graph[resource_id][neighbor].values():
                    if isinstance(edge_data['data'], Relationship):
                        rel = edge_data['data']
                        if rel.kind in [RelationshipKind.PRODUCES, RelationshipKind.USES, 
    ...
    def identify_bottlenecks(self, flow_type: FlowNature) -> List[uuid.UUID]:
        ...
        bottlenecks = [node_id for node_id, score in centrality.items() 
                      if score >= threshold]
    ...
    def calculate_flow_efficiency(self, source_id: uuid.UUID, target_id: uuid.UUID) -> float:
        """Calculate efficiency of flows between nodes."""
        try:
            shortest_path_length = len(self.find_shortest_path(source_id, target_id) or []) - 1
    ...
    def analyze_policy_impact(self, policy_id: uuid.UUID, 
        ...
        try:
            ...
            impact_analysis = {
                'total_affected_nodes': len(affected_nodes) - 1,  # Exclude policy node itself
                'affected_actors': [],
                'affected_institutions': [],
                'affected_resources': [],
                'network_metrics': {
                    'density': nx.density(ego_graph),
                    'centrality': nx.betweenness_centrality(ego_graph).get(policy_id, 0.0)
                }
            }
    ...
    def compare_policy_scenarios(self, scenario_graphs: List[SFMGraph]) -> Dict[str, Any]:
        ...
        comparison = {
            ...
            'relationship_count_comparison': [len(g.relationships) for g in scenario_graphs]
        }
        return comparison
    ...
    def get_structural_holes(self) -> List[uuid.UUID]:
        ...
        structural_bridges = [node_id for node_id, score in centrality.items() 
                            if score >= threshold]
    ...
    def system_vulnerability_analysis(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            'average_clustering': nx.average_clustering(self.nx_graph.to_undirected()),
...
class SFMQueryFactory:
    ...
    @staticmethod
    def create_query_engine(graph: SFMGraph, backend: str = "networkx") -> SFMQueryEngine:
